...
function chaxunshuju() {
    ...
    $.post("../tool/CreateTable.ashx", { "siteid": compatibility }, function (data) {
        var dv = document.getElementById('myzhanshi');
...
function loaded() {
    $.post("../tool/CreateControl.ashx", { "siteid": "061451288780" }, function (data) {
       // alert(data);
        var dv = document.getElementById('mytb');
        dv.innerHTML =data;
    }, "json");
}

function tanchuang(zfc) {
...
//3.综合数据——根据基站编码获得指定基站的实时监控数据项目（定时器，循环所有基站）
function GetSiteData2() {
    var k = document.getElementById("from").value;
    var j = document.getElementById("to").value;
    if (form1.NetNo == '') return;
    var Net = form1.NetNo.split(';');
    //alert(k+'ss'+j)
    var zc=1;
    for (var nIdx = 0; nIdx < Net.length; nIdx++) {
        ...
        $.post("../tool/Getribao.ashx", { "siteid": Net[nIdx], "beginDate": k, "endDate": j,"zcc":zc }, pageFinish2, "json");
        zc++;
